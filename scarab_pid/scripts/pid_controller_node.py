#!/usr/bin/env python3
"""
ROS node for the Scarab PID trajectory controller.

Each node instance controls one robot: subscribe to /<agent>/pose and
/<agent>/move/goal, track externally published waypoints, and publish
/<agent>/cmd_vel directly. Multiple instances can run in parallel under
different robot namespaces. The node does not start or call HFN.

This is the PID counterpart of scarab_mpc/scripts/mpc_controller_node.py.
The two are independent implementations that expose the same ROS interface,
so the same waypoint publisher (pub_traj_test_real.py, pub_traj_dual.py)
drives either one without modification. Never run both on the same robot at
the same time: they would both publish /<agent>/cmd_vel.
"""

from __future__ import annotations

import json
import math
import os
import threading
import time as wall_time
from datetime import datetime
from typing import Any, Dict, List, Optional, Tuple

import numpy as np
import rospy
from geometry_msgs.msg import PoseStamped, Twist
from nav_msgs.msg import Path
from scarab_msgs.msg import MoveActionGoal
from std_msgs.msg import String

from scarab_pid.config import PIDConfig
from scarab_pid.pid_controller import PIDTrajectoryController
from scarab_pid.reference_generator import ReferenceTrajectory
from scarab_pid.ros_adapter import (
    make_twist,
    normalize_angle,
    pose_to_state,
    yaw_to_quaternion,
    zero_twist,
)


def _param(name: str, default: Any = None) -> Any:
    return rospy.get_param("~" + name, default)


def _as_bool(value: Any) -> bool:
    if isinstance(value, bool):
        return value
    if isinstance(value, str):
        return value.strip().lower() in ("1", "true", "yes", "on")
    return bool(value)


def _parse_start_time(value: Any) -> Optional[rospy.Time]:
    """Accept empty, epoch seconds, or 'YYYY-MM-DD HH:MM:SS'."""
    if value is None:
        return None

    if isinstance(value, (int, float)):
        return rospy.Time.from_sec(float(value))

    text = str(value).strip()
    if not text:
        return None

    try:
        return rospy.Time.from_sec(float(text))
    except ValueError:
        pass

    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M:%S"):
        try:
            parsed = datetime.strptime(text, fmt)
            return rospy.Time.from_sec(wall_time.mktime(parsed.timetuple()))
        except ValueError:
            continue

    raise ValueError(
        "~start_time must be empty, epoch seconds, or 'YYYY-MM-DD HH:MM:SS'"
    )


def _heading_between(points: List[List[float]], index: int) -> float:
    if len(points) < 2:
        return 0.0
    current = points[index]
    for next_index in range(index + 1, len(points)):
        candidate = points[next_index]
        dx = candidate[0] - current[0]
        dy = candidate[1] - current[1]
        if math.hypot(dx, dy) > 1e-8:
            return math.atan2(dy, dx)
    for prev_index in range(index - 1, -1, -1):
        candidate = points[prev_index]
        dx = current[0] - candidate[0]
        dy = current[1] - candidate[1]
        if math.hypot(dx, dy) > 1e-8:
            return math.atan2(dy, dx)
    return 0.0


def _make_path_from_waypoints(
    waypoints: List[List[float]],
    frame_id: str,
    stamp: rospy.Time,
) -> Path:
    path = Path()
    path.header.stamp = stamp
    path.header.frame_id = frame_id
    for index, point in enumerate(waypoints):
        pose = PoseStamped()
        pose.header = path.header
        pose.pose.position.x = float(point[0])
        pose.pose.position.y = float(point[1])
        pose.pose.position.z = 0.0
        qx, qy, qz, qw = yaw_to_quaternion(_heading_between(waypoints, index))
        pose.pose.orientation.x = qx
        pose.pose.orientation.y = qy
        pose.pose.orientation.z = qz
        pose.pose.orientation.w = qw
        path.poses.append(pose)
    return path


def _make_path_from_states(
    states: np.ndarray,
    frame_id: str,
    stamp: rospy.Time,
) -> Path:
    path = Path()
    path.header.stamp = stamp
    path.header.frame_id = frame_id
    for state in states:
        pose = PoseStamped()
        pose.header = path.header
        pose.pose.position.x = float(state[0])
        pose.pose.position.y = float(state[1])
        pose.pose.position.z = 0.0
        qx, qy, qz, qw = yaw_to_quaternion(float(state[2]))
        pose.pose.orientation.x = qx
        pose.pose.orientation.y = qy
        pose.pose.orientation.z = qz
        pose.pose.orientation.w = qw
        path.poses.append(pose)
    return path


class ScarabPIDNode:
    def __init__(self) -> None:
        self.agent = str(_param("agent", os.environ.get("AGENT", "scarab42")))
        self.robot_index = int(_param("robot_index", 0))

        self.dry_run = _as_bool(_param("dry_run", True))
        self.publish_cmd = _as_bool(_param("publish_cmd", False))
        self.stop_on_exit = _as_bool(_param("stop_on_exit", True))
        self.pose_timeout = float(_param("pose_timeout", 0.5))
        self.control_rate = float(_param("control_rate", 20.0))
        self.start_delay = float(_param("start_delay", 0.0))
        self.start_time = _parse_start_time(_param("start_time", ""))
        self.prepend_current_pose = _as_bool(_param("prepend_current_pose", True))
        self.prepend_current_pose_min_distance = float(
            _param("prepend_current_pose_min_distance", 0.05)
        )
        self.use_goal_stamps = _as_bool(_param("use_goal_stamps", True))

        self.pose_topic = str(_param("pose_topic", f"/{self.agent}/pose"))
        self.cmd_vel_topic = str(_param("cmd_vel_topic", f"/{self.agent}/cmd_vel"))
        self.goal_topic = str(_param("goal_topic", f"/{self.agent}/move/goal"))
        self.status_topic = str(_param("status_topic", f"/{self.agent}/pid/status"))
        self.predicted_path_topic = str(
            _param("predicted_path_topic", f"/{self.agent}/pid/predicted_path")
        )
        self.reference_path_topic = str(
            _param("reference_path_topic", f"/{self.agent}/pid/reference_path")
        )
        self.actual_path_topic = str(
            _param("actual_path_topic", f"/{self.agent}/pid/actual_path")
        )
        self.actual_path_min_distance = float(_param("actual_path_min_distance", 0.03))
        self.actual_path_max_poses = int(_param("actual_path_max_poses", 2000))
        self.tracking_error_log_period = float(_param("tracking_error_log_period", 1.0))
        self.frame_id = str(_param("frame_id", f"{self.agent}/map"))

        self.cfg = self._load_pid_config()
        self.controller = PIDTrajectoryController(
            robot_id=self.robot_index,
            cfg=self.cfg,
        )

        self.reference: Optional[ReferenceTrajectory] = None
        self.reference_waypoints: List[List[float]] = []
        self.active_goal_id = ""
        self.pending_goal: Optional[MoveActionGoal] = None
        self.current_state: Optional[np.ndarray] = None
        self.last_pose_time: Optional[rospy.Time] = None
        self.first_ready_time: Optional[rospy.Time] = None
        self.tracking_start_time: Optional[rospy.Time] = None
        self.goal_start_time: Optional[rospy.Time] = None
        self.finished = False
        self.actual_path = Path()
        self.actual_path.header.frame_id = self.frame_id
        self._last_step_time: Optional[rospy.Time] = None
        self._state_lock = threading.RLock()

        self.cmd_pub = rospy.Publisher(self.cmd_vel_topic, Twist, queue_size=1)
        self.status_pub = rospy.Publisher(self.status_topic, String, queue_size=1)
        self.predicted_path_pub = rospy.Publisher(
            self.predicted_path_topic, Path, queue_size=1
        )
        self.reference_path_pub = rospy.Publisher(
            self.reference_path_topic, Path, queue_size=1, latch=True
        )
        self.actual_path_pub = rospy.Publisher(
            self.actual_path_topic, Path, queue_size=1, latch=True
        )
        self.pose_sub = rospy.Subscriber(
            self.pose_topic, PoseStamped, self._on_pose, queue_size=1
        )
        self.goal_sub = rospy.Subscriber(
            self.goal_topic, MoveActionGoal, self._on_goal, queue_size=1
        )

        rospy.on_shutdown(self._on_shutdown)
        self._log_startup()

    # ------------------------------------------------------------------
    # Configuration
    # ------------------------------------------------------------------
    def _load_pid_config(self) -> PIDConfig:
        default = PIDConfig()
        return PIDConfig(
            dt=float(_param("dt", default.dt)),
            v_min=float(_param("v_min", default.v_min)),
            v_max=float(_param("v_max", default.v_max)),
            omega_max=float(_param("omega_max", default.omega_max)),
            cruise_speed=float(_param("cruise_speed", default.cruise_speed)),
            turn_angle_threshold=float(
                _param("turn_angle_threshold", default.turn_angle_threshold)
            ),
            kp_along=float(_param("kp_along", default.kp_along)),
            ki_along=float(_param("ki_along", default.ki_along)),
            kd_along=float(_param("kd_along", default.kd_along)),
            kp_cross=float(_param("kp_cross", default.kp_cross)),
            ki_cross=float(_param("ki_cross", default.ki_cross)),
            kd_cross=float(_param("kd_cross", default.kd_cross)),
            kp_heading=float(_param("kp_heading", default.kp_heading)),
            ki_heading=float(_param("ki_heading", default.ki_heading)),
            kd_heading=float(_param("kd_heading", default.kd_heading)),
            i_max_along=float(_param("i_max_along", default.i_max_along)),
            i_max_cross=float(_param("i_max_cross", default.i_max_cross)),
            i_max_heading=float(_param("i_max_heading", default.i_max_heading)),
            d_filter_tau=float(_param("d_filter_tau", default.d_filter_tau)),
            a_max=float(_param("a_max", default.a_max)),
            alpha_max=float(_param("alpha_max", default.alpha_max)),
            align_angle=float(_param("align_angle", default.align_angle)),
            align_speed_factor=float(
                _param("align_speed_factor", default.align_speed_factor)
            ),
            goal_tolerance=float(_param("goal_tolerance", default.goal_tolerance)),
            predict_steps=int(_param("predict_steps", default.predict_steps)),
        )

    # ------------------------------------------------------------------
    # Callbacks
    # ------------------------------------------------------------------
    def _on_goal(self, msg: MoveActionGoal) -> None:
        with self._state_lock:
            if msg.goal.stop:
                self._clear_reference("stop_goal")
                return

            if not msg.goal.target_poses:
                rospy.logwarn(
                    "[%s] Ignoring empty goal on %s", self.agent, self.goal_topic
                )
                return

            if self.current_state is None:
                self.pending_goal = msg
                rospy.loginfo(
                    "[%s] Queued goal with %d target pose(s); waiting for pose.",
                    self.agent,
                    len(msg.goal.target_poses),
                )
                return

            self._accept_goal(msg)

    def _goal_stamps(self, msg: MoveActionGoal) -> Optional[List[float]]:
        """Absolute arrival times carried by the goal, or None.

        A timestamped trajectory encodes one arrival time per target pose in
        PoseStamped.header.stamp. Publishers that do not do this (the older
        pub_traj_test_real.py stamps every pose identically) fail the
        strictly-increasing test and fall back to cruise_speed timing, so this
        stays backward compatible.
        """
        if not self.use_goal_stamps:
            return None

        stamps = [p.header.stamp.to_sec() for p in msg.goal.target_poses]
        if any(s <= 0.0 for s in stamps):
            return None
        if any(stamps[i + 1] <= stamps[i] for i in range(len(stamps) - 1)):
            return None
        return stamps

    def _timeline_from_stamps(
        self,
        stamps: List[float],
        waypoints: List[List[float]],
        prepended: bool,
        now: rospy.Time,
    ) -> Tuple[Optional[np.ndarray], Optional[rospy.Time]]:
        """Turn absolute arrival times into (segment_times, t_ref origin).

        When the robot's current pose was prepended, that extra leading point
        is given the time remaining until the first commanded stamp, so the
        robot arrives at the trajectory start exactly on time and every later
        waypoint still lands on its absolute stamp. Two robots fed the same
        stamps therefore reach waypoint k together no matter how far apart
        they started or how differently long their paths are.
        """
        arrival = ([now.to_sec()] + list(stamps)) if prepended else list(stamps)

        if len(arrival) != len(waypoints):
            rospy.logwarn(
                "[%s] Goal stamps (%d) do not line up with waypoints (%d); "
                "falling back to cruise_speed timing.",
                self.agent, len(arrival), len(waypoints),
            )
            return None, None

        segment_times = np.diff(np.array(arrival, dtype=np.float64))
        if segment_times.size == 0:
            return None, None

        if float(segment_times.min()) <= 0.0:
            late_by = -float(segment_times.min())
            rospy.logwarn(
                "[%s] Goal stamps are not reachable: first arrival is %.2fs in "
                "the past. Clamping that segment; the robot will chase the "
                "schedule. Publish with more lead time.",
                self.agent, late_by,
            )
            segment_times = np.maximum(segment_times, self.cfg.dt)

        seg_lengths = np.linalg.norm(
            np.diff(np.asarray(waypoints, dtype=np.float64), axis=0), axis=1
        )
        required = seg_lengths / np.maximum(segment_times, 1e-9)
        worst = int(np.argmax(required))
        if float(required[worst]) > self.cfg.v_max + 1e-9:
            rospy.logwarn(
                "[%s] Goal stamps demand %.3f m/s on segment %d (%.2f m in "
                "%.2f s) but v_max is %.3f m/s. The robot cannot hold this "
                "schedule and will run late from the start.",
                self.agent,
                float(required[worst]),
                worst,
                float(seg_lengths[worst]),
                float(segment_times[worst]),
                self.cfg.v_max,
            )

        return segment_times, rospy.Time.from_sec(arrival[0])

    def _accept_goal(self, msg: MoveActionGoal) -> None:
        target_waypoints = self._waypoints_from_goal(msg)
        if not target_waypoints:
            rospy.logwarn("[%s] Ignoring goal with no usable target poses.", self.agent)
            return

        now = rospy.Time.now()
        waypoints = self._waypoints_with_current_start(target_waypoints)
        prepended = len(waypoints) == len(target_waypoints) + 1

        segment_times = None
        self.goal_start_time = None
        stamps = self._goal_stamps(msg)
        if stamps is not None:
            segment_times, self.goal_start_time = self._timeline_from_stamps(
                stamps, waypoints, prepended, now
            )

        self.reference = ReferenceTrajectory(
            waypoints, self.cfg, segment_times=segment_times
        )
        self.reference_waypoints = waypoints
        self.active_goal_id = msg.goal_id.id or f"goal_{msg.goal_id.stamp.to_nsec()}"
        self.pending_goal = None
        self.first_ready_time = None
        self.tracking_start_time = None
        self.finished = False
        self._reset_controller()
        # Fresh actual-path recording per goal: without this the RViz /
        # plot_tracking overlay accumulates every previous run in the node's
        # lifetime and corrupts the path-deviation numbers.
        self.actual_path.poses = []
        self.actual_path_pub.publish(self.actual_path)
        self._publish_reference_path()

        rospy.loginfo(
            "[%s] Accepted external PID goal id=%s targets=%d waypoints=%d "
            "total_time=%.2fs timing=%s source=%s",
            self.agent,
            self.active_goal_id,
            len(target_waypoints),
            len(self.reference_waypoints),
            self.reference.total_time,
            "goal_stamps" if segment_times is not None else "cruise_speed",
            self.goal_topic,
        )
        if self.goal_start_time is not None:
            rospy.loginfo(
                "[%s] Timed trajectory: t_ref origin=%.3f, finishes at %.3f "
                "(in %.2fs).",
                self.agent,
                self.goal_start_time.to_sec(),
                self.goal_start_time.to_sec() + self.reference.total_time,
                self.goal_start_time.to_sec() + self.reference.total_time
                - now.to_sec(),
            )

    def _clear_reference(self, reason: str) -> None:
        self.reference = None
        self.reference_waypoints = []
        self.active_goal_id = ""
        self.pending_goal = None
        self.first_ready_time = None
        self.tracking_start_time = None
        self.goal_start_time = None
        self.finished = False
        self._reset_controller()
        self._publish_reference_path()
        self._publish_stop(reason)
        rospy.loginfo("[%s] Cleared PID reference: %s", self.agent, reason)

    def _waypoints_from_goal(self, msg: MoveActionGoal) -> List[List[float]]:
        waypoints: List[List[float]] = []
        for pose_stamped in msg.goal.target_poses:
            frame_id = pose_stamped.header.frame_id or msg.header.frame_id
            if frame_id and frame_id != self.frame_id:
                rospy.logwarn_throttle(
                    5.0,
                    "[%s] Goal frame '%s' differs from PID frame '%s'. "
                    "No TF transform is applied; coordinates are used as-is.",
                    self.agent,
                    frame_id,
                    self.frame_id,
                )
            waypoints.append([
                float(pose_stamped.pose.position.x),
                float(pose_stamped.pose.position.y),
            ])
        return waypoints

    def _waypoints_with_current_start(
        self,
        target_waypoints: List[List[float]],
    ) -> List[List[float]]:
        """Drive from where the robot actually is toward the first waypoint."""
        if self.current_state is None:
            return target_waypoints

        current = [float(self.current_state[0]), float(self.current_state[1])]
        first = target_waypoints[0]
        first_distance = math.hypot(current[0] - first[0], current[1] - first[1])

        if len(target_waypoints) == 1:
            return [current, first]

        if (
            self.prepend_current_pose
            and first_distance > self.prepend_current_pose_min_distance
        ):
            return [current] + target_waypoints

        return target_waypoints

    def _on_pose(self, msg: PoseStamped) -> None:
        with self._state_lock:
            self.current_state = pose_to_state(msg)
            self.last_pose_time = rospy.Time.now()
            self._record_actual_path(msg)

    def _record_actual_path(self, msg: PoseStamped) -> None:
        frame_id = msg.header.frame_id or self.frame_id
        stamp = (
            msg.header.stamp
            if msg.header.stamp != rospy.Time(0)
            else rospy.Time.now()
        )
        if not self.actual_path.poses:
            self.actual_path.header.frame_id = frame_id
        elif self.actual_path_min_distance > 0.0:
            last = self.actual_path.poses[-1].pose.position
            current = msg.pose.position
            if math.hypot(current.x - last.x, current.y - last.y) < \
                    self.actual_path_min_distance:
                return

        pose = PoseStamped()
        pose.header.stamp = stamp
        pose.header.frame_id = self.actual_path.header.frame_id
        pose.pose.position.x = msg.pose.position.x
        pose.pose.position.y = msg.pose.position.y
        pose.pose.position.z = msg.pose.position.z
        pose.pose.orientation = msg.pose.orientation

        self.actual_path.poses.append(pose)
        if self.actual_path_max_poses > 0 and \
                len(self.actual_path.poses) > self.actual_path_max_poses:
            self.actual_path.poses = self.actual_path.poses[-self.actual_path_max_poses:]
        self.actual_path.header.stamp = stamp
        self.actual_path_pub.publish(self.actual_path)

    # ------------------------------------------------------------------
    # Control loop
    # ------------------------------------------------------------------
    def spin(self) -> None:
        rate = rospy.Rate(self.control_rate)
        while not rospy.is_shutdown():
            with self._state_lock:
                self._step()
            rate.sleep()

    def _step(self) -> None:
        now = rospy.Time.now()
        if self.current_state is None or self.last_pose_time is None:
            self._publish_status(
                "waiting_for_pose",
                now,
                pending_goal=self.pending_goal is not None,
            )
            rospy.logwarn_throttle(
                2.0, "[%s] Waiting for pose on %s", self.agent, self.pose_topic
            )
            return

        if self.pending_goal is not None:
            self._accept_goal(self.pending_goal)

        pose_age = (now - self.last_pose_time).to_sec()
        if self.pose_timeout > 0.0 and pose_age > self.pose_timeout:
            self._reset_controller()
            self._publish_stop("stale_pose")
            self._publish_status("stale_pose", now, pose_age=pose_age)
            rospy.logwarn_throttle(
                1.0,
                "[%s] Pose timeout: age=%.3fs > %.3fs. Publishing stop.",
                self.agent,
                pose_age,
                self.pose_timeout,
            )
            return

        if self.reference is None:
            self._publish_stop("waiting_for_goal")
            self._publish_status("waiting_for_goal", now, pose_age=pose_age)
            rospy.loginfo_throttle(
                2.0,
                "[%s] Waiting for external waypoint goal on %s",
                self.agent,
                self.goal_topic,
            )
            return

        if self.tracking_start_time is None:
            if self.first_ready_time is None:
                self.first_ready_time = now
            # A timed trajectory carries its own absolute origin, which wins
            # over ~start_time: the schedule is already fully specified.
            origin = (
                self.goal_start_time
                if self.goal_start_time is not None
                else self.start_time
            )
            if origin is not None:
                delay_remaining = (origin - now).to_sec()
                if delay_remaining > 0.0:
                    self._publish_stop("start_time")
                    self._publish_status(
                        "start_time",
                        now,
                        delay_remaining=delay_remaining,
                        start_time=origin.to_sec(),
                    )
                    return
                self.tracking_start_time = origin
                self._reset_controller()
                rospy.loginfo(
                    "[%s] PID tracking started at synchronized origin %.3f (%s).",
                    self.agent,
                    origin.to_sec(),
                    "goal_stamps" if self.goal_start_time is not None
                    else "~start_time",
                )
                return
            ready_age = (now - self.first_ready_time).to_sec()
            if ready_age < self.start_delay:
                self._publish_stop("start_delay")
                self._publish_status(
                    "start_delay", now, delay_remaining=self.start_delay - ready_age
                )
                return
            self.tracking_start_time = now
            self._reset_controller()
            rospy.loginfo("[%s] PID tracking started.", self.agent)

        t_ref = (now - self.tracking_start_time).to_sec()
        if self.finished:
            self._publish_stop("finished")
            self._publish_status("finished", now, t_ref=t_ref)
            return

        # Past total_time the reference pins the goal with zero feedforward,
        # so the controller keeps running here and ramps the command down
        # through its slew limits instead of stepping cmd_vel to zero.
        x_ref, u_ref = self.reference.query(t_ref)
        tracking_error = self._tracking_error(x_ref)

        step_dt = None
        if self._last_step_time is not None:
            delta = (now - self._last_step_time).to_sec()
            if delta > 0.0:
                step_dt = delta
        self._last_step_time = now

        v_cmd, omega_cmd = self.controller.compute(
            self.current_state, x_ref, u_ref, dt=step_dt
        )

        if self.dry_run:
            rospy.loginfo_throttle(
                1.0,
                "[%s] DRY RUN t=%.2f progress=%.1f%% cmd=(%.3f, %.3f)",
                self.agent,
                t_ref,
                100.0 * self.reference.get_progress(t_ref),
                v_cmd,
                omega_cmd,
            )
        elif not self.publish_cmd:
            rospy.loginfo_throttle(
                2.0,
                "[%s] publish_cmd=false; computed cmd=(%.3f, %.3f)",
                self.agent,
                v_cmd,
                omega_cmd,
            )
        else:
            self.cmd_pub.publish(make_twist(v_cmd, omega_cmd))

        # Declare finished only once the command has ramped down as well, so
        # the transition to the standing stop is a no-op, not a velocity step
        # that would bypass the a_max slew limit.
        if (
            self.reference.is_finished(
                t_ref, self.current_state, self.cfg.goal_tolerance
            )
            and abs(v_cmd) < 0.02
            and abs(omega_cmd) < 0.05
        ):
            rospy.loginfo("[%s] PID trajectory finished.", self.agent)
            self.finished = True
            self._reset_controller()
            self._publish_stop("finished")
            self._publish_status("finished", now, t_ref=t_ref)
            return

        if self.tracking_error_log_period > 0.0:
            stats = self.controller.stats
            rospy.loginfo_throttle(
                self.tracking_error_log_period,
                (
                    "[%s] tracking_error xy=%.3f along=%.3f cross=%.3f "
                    "theta=%.3f | i_along=%.4f | ref=(%.3f, %.3f, %.3f) "
                    "pose=(%.3f, %.3f, %.3f) cmd=(%.3f, %.3f)"
                ),
                self.agent,
                tracking_error["tracking_error_xy"],
                stats.get("e_along", 0.0),
                stats.get("e_cross", 0.0),
                tracking_error["tracking_error_theta"],
                stats.get("i_along", 0.0),
                tracking_error["reference_x"],
                tracking_error["reference_y"],
                tracking_error["reference_theta"],
                tracking_error["pose_x"],
                tracking_error["pose_y"],
                tracking_error["pose_theta"],
                v_cmd,
                omega_cmd,
            )

        self._publish_predicted_path(now)
        self._publish_status(
            "tracking",
            now,
            t_ref=t_ref,
            pose_age=pose_age,
            v=v_cmd,
            omega=omega_cmd,
            **tracking_error,
        )

    # ------------------------------------------------------------------
    # Helpers
    # ------------------------------------------------------------------
    def _reset_controller(self) -> None:
        """Reset controller state and the control-period clock together."""
        self.controller.reset()
        self._last_step_time = None

    def _tracking_error(self, ref_state: np.ndarray) -> Dict[str, float]:
        if self.current_state is None:
            return {}
        err_x = float(self.current_state[0] - ref_state[0])
        err_y = float(self.current_state[1] - ref_state[1])
        err_theta = float(normalize_angle(self.current_state[2] - ref_state[2]))
        return {
            "pose_x": float(self.current_state[0]),
            "pose_y": float(self.current_state[1]),
            "pose_theta": float(self.current_state[2]),
            "reference_x": float(ref_state[0]),
            "reference_y": float(ref_state[1]),
            "reference_theta": float(ref_state[2]),
            "tracking_error_x": err_x,
            "tracking_error_y": err_y,
            "tracking_error_theta": err_theta,
            "tracking_error_xy": float(math.hypot(err_x, err_y)),
        }

    def _publish_stop(self, reason: str) -> None:
        if not self.dry_run and self.publish_cmd:
            self.cmd_pub.publish(zero_twist())
        rospy.loginfo_throttle(2.0, "[%s] stop command: %s", self.agent, reason)

    def _publish_status(self, mode: str, stamp: rospy.Time, **extra: Any) -> None:
        payload: Dict[str, Any] = {
            "agent": self.agent,
            "controller_type": "pid",
            "mode": mode,
            "goal_topic": self.goal_topic,
            "active_goal_id": self.active_goal_id,
            "has_reference": self.reference is not None,
            "waypoint_count": len(self.reference_waypoints),
            "dry_run": self.dry_run,
            "publish_cmd": self.publish_cmd,
            "stamp": stamp.to_sec(),
            "progress": None,
            "solve": self.controller.stats,
        }
        if self.reference is not None and self.tracking_start_time is not None:
            t_ref = (stamp - self.tracking_start_time).to_sec()
            payload["progress"] = float(self.reference.get_progress(t_ref))
        for key, value in extra.items():
            if isinstance(value, np.generic):
                value = value.item()
            payload[key] = value
        self.status_pub.publish(String(data=json.dumps(payload, sort_keys=True)))

    def _publish_reference_path(self) -> None:
        stamp = rospy.Time.now()
        if self.reference_waypoints:
            path = _make_path_from_waypoints(
                self.reference_waypoints, self.frame_id, stamp
            )
        else:
            path = Path()
            path.header.stamp = stamp
            path.header.frame_id = self.frame_id
        self.reference_path_pub.publish(path)

    def _publish_predicted_path(self, stamp: rospy.Time) -> None:
        states = self.controller.last_predicted_states
        if states is None:
            return
        self.predicted_path_pub.publish(
            _make_path_from_states(states, self.frame_id, stamp)
        )

    def _on_shutdown(self) -> None:
        if self.stop_on_exit:
            self._publish_stop("shutdown")

    def _log_startup(self) -> None:
        cfg = self.cfg
        rospy.loginfo(
            "[%s] Scarab PID node ready. pose=%s cmd_vel=%s goal=%s "
            "dry_run=%s publish_cmd=%s rate=%.1fHz",
            self.agent,
            self.pose_topic,
            self.cmd_vel_topic,
            self.goal_topic,
            self.dry_run,
            self.publish_cmd,
            self.control_rate,
        )
        rospy.loginfo(
            "[%s] PID gains along=(%.3f, %.3f, %.3f) cross=(%.3f, %.3f, %.3f) "
            "heading=(%.3f, %.3f, %.3f)",
            self.agent,
            cfg.kp_along, cfg.ki_along, cfg.kd_along,
            cfg.kp_cross, cfg.ki_cross, cfg.kd_cross,
            cfg.kp_heading, cfg.ki_heading, cfg.kd_heading,
        )
        rospy.loginfo(
            "[%s] cruise=%.3f v_max=%.3f omega_max=%.3f a_max=%.2f "
            "alpha_max=%.2f corner_threshold=%.2frad",
            self.agent,
            cfg.cruise_speed,
            cfg.v_max,
            cfg.omega_max,
            cfg.a_max,
            cfg.alpha_max,
            cfg.turn_angle_threshold,
        )

        if cfg.cruise_speed >= cfg.v_max:
            rospy.logwarn(
                "[%s] ~cruise_speed=%.3f >= ~v_max=%.3f leaves the controller "
                "no authority to correct lag. Lower cruise_speed.",
                self.agent,
                cfg.cruise_speed,
                cfg.v_max,
            )
        if abs(self.control_rate * cfg.dt - 1.0) > 0.25:
            rospy.logwarn(
                "[%s] ~control_rate=%.1fHz does not match ~dt=%.3fs "
                "(implies %.1fHz). The reference sampling grid will not line "
                "up with the actual loop period.",
                self.agent,
                self.control_rate,
                cfg.dt,
                1.0 / cfg.dt if cfg.dt > 0 else float("nan"),
            )
        if self.dry_run or not self.publish_cmd:
            rospy.logwarn(
                "[%s] Motor publishing disabled by dry_run=%s publish_cmd=%s.",
                self.agent,
                self.dry_run,
                self.publish_cmd,
            )
        if self.start_time is not None:
            rospy.loginfo(
                "[%s] PID synchronized start_time=%.3f.",
                self.agent,
                self.start_time.to_sec(),
            )


def main() -> None:
    rospy.init_node("pid_controller")
    node = ScarabPIDNode()
    node.spin()


if __name__ == "__main__":
    try:
        main()
    except rospy.ROSInterruptException:
        pass
