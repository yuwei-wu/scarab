...
void TrajectoryGenerator::setInitialConditions(const Vec2f &pos, const vec_Vec2f &derivatives)
{
  clearWaypoints();
...
void TrajectoryGenerator::addWaypoint(const Vec2f &x)
{
  if(waypoints_.empty())
    setInitialConditions(x, vec_Vec2f());
  else
...
std::vector<float> TrajectoryGenerator::computeTimesTrapezoidSpeed(float v_des, float a_des) const
{
  ...
  const Vec2f &initial_vel_ = initial_derivatives_[0];
  ...
  //@yuwei: the original equation does not work for nonzero start  v = v_initial + at
  const float t_accel = (v_des - v_initial) / a_des;
  ...
  for(unsigned int i = 1; i < waypoints_.size(); ++i)
  {
    if(accumulated_dist[i] <= d_accel)  // Accel
    {
      waypoint_times.push_back((std::sqrt(v_initial * v_initial + 2 * a_des * accumulated_dist[i]) - v_initial) /
                               a_des);
      std::cout << waypoint_times.back() << std::endl;
    }
...
bool TrajectoryGenerator::calculate(const std::vector<float> &waypoint_times)
{
  ...
  // Fixed derivatives
  Eigen::MatrixX2f Df = Eigen::MatrixX2f(num_fixed_derivatives, 2);
  // First point
  ...
  for(unsigned int i = 1; i < N_ / 2; i++)
  {
    Df.row((N_ / 2) + (num_waypoints - 2) + i) = Vec2f::Zero().transpose();
  }
  // std::cout << "Df:\n" << Df << std::endl;
  Eigen::MatrixX2f D = Eigen::MatrixX2f(num_waypoints * N_ / 2, 2);
  D.topRows(num_fixed_derivatives) = Df;
  if(num_waypoints > 2 && num_free_derivatives > 0)
  {
    Eigen::MatrixX2f Dp = -Rpp.partialPivLu().solve(Rpf * Df);
    // std::cout << "Dp:\n" << Dp << std::endl;
                                            ...
                                            std::vector<float> &max_jrk) const
{
  ...
  for(unsigned int seg_idx = 0; seg_idx < waypoint_times_.size() - 1; ++seg_idx)
  {
    ...
    for(unsigned int sample_idx = 0; sample_idx < num_samples_per_seg; ++sample_idx)
    {
      ...
      const Eigen::MatrixX2f &p = coefficients_[seg_idx];
...
bool Trajectory::getCommand(const float time, Vec2f &pos, Vec2f &vel, Vec2f &acc, Vec2f &jrk) const
{
...
bool Trajectory::getPosition(const float time, Vec2f &pos) const
{
  if (time > waypoint_times_.back())
  {
    const float t_traj = waypoint_times_.back() - waypoint_times_[waypoint_times_.size() - 2];
    const Eigen::MatrixX2f &p = coefficients_.back();
    pos = Vec2f::Zero();
    for(unsigned int i = 0; i < p.rows(); i++)
      pos += p.row(i).transpose() * powInt(t_traj, i);
...
bool Trajectory::getVelocity(const float time, Vec2f &vel) const
{ 
  if (time > waypoint_times_.back())
  {
    vel = Vec2f::Zero();
...
bool Trajectory::getAcceleration(const float time, Vec2f &acc) const
{
  if (time > waypoint_times_.back())
  {
    acc = Vec2f::Zero();
...
int Trajectory::getIndex(const float time) const
{
