...
class MoveToGoal(object):
    ...
    def changing_frames(msg, name):
	quat = tft.numpy.array([data.pose.orientation.x,
                                data.pose.orientation.y,
                                data.pose.orientation.z,
                                data.pose.orientation.w])
	br=tf.TransformBroadcaster(),
	br.sendTransform((msg.x, msg.y, 0), tft.euler_from_quaternion(quat),  rospy.Time.now(), name, "world")
    ...
    def pose_cb(self, data):
		...
		self._angle_b=np.arctan2((self._goal_y-y),(self._goal_x-x))
